...
function [con,Velocity_s,KValue] = C4Ini(params,kvalues,vmax)%(Begin, GenNum)
factorvp=params(3);
factorvc=params(4);
vpmax=vmax(2);
vcmax=vmax(6);
...
Velocity_s=vmax;
Velocity_s([2,6])=[Vm_2,Vm_6];

% Velocity_s=zeros((length(vmax)+2),1); 
% 
% Velocity_s(setdiff(1:length(Velocity_s),[2,6]))=vmax;
% Velocity_s([2,6])=[Vm_2,Vm_6];
