...
vmeanind=find(contains(param_name,'Vm1'),1):find(contains(param_name,'Vm35_Hep'));


% parameters22=Variable(1:nvar,1);
% parameters23=parameters22;
% parameters23(vmeanind)=Variable((nvar+1):end);

parameters0=zeros((length(init_sol)+length(vmeanind)),1);
parameters0(estimated_ind)=Variable;
parameters22=parameters0(1:length(init_sol));
parameters23=parameters22;
parameters23(vmeanind)=parameters0((length(init_sol)+1):end);
...
% meanAstd=mean([ACa22.sd(:,argind);ACa23.sd(:,argind)]);
% meanGsstd=mean([GsCa22.sd(:,argind);GsCa23.sd(:,argind)]);
% meanAQstd=mean([AQ22.sd(:,argind);AQ23.sd(:,argind)]);
% 
% zACI22=sum((ACa22.meas(:,argind)-simA22).^2./meanAstd^2,'omitnan');
% zgs22=sum((GsCa22.meas(:,argind)-simgs22).^2./meanGsstd^2,'omitnan');
% 
% zACI23=sum((ACa23.meas(:,argind)-simA23).^2./meanAstd^2,'omitnan');
% zgs23=sum((GsCa23.meas(:,argind)-simgs23).^2./meanGsstd^2,'omitnan');
% 
% zAQ22=sum((AQ22.meas(:,argind)-simAQ22).^2./meanAstd^2,'omitnan');
% zAQ23=sum((AQ23.meas(:,argind)-simAQ23).^2./meanAstd^2,'omitnan');
