function [simA,simGs] = simulate_ACI(Variable,Tair,KE_type)
...
while continue_flag && i<(nchange+1)
    ...
    if continue_flag==true
        ...
        if i==1
            temp=[Gs_VEL(:,1),Gs_VEL(:,ind_A)];
            temp2=[ode_sol.x',ode_sol.y(ind_gs,:)'];
            photo_rate=temp;
            gs_conc=temp2;
            time_count=Gs_VEL(end,1);
            time_count2=ode_sol.x(end);
        else
            temp=[Gs_VEL(:,1)+time_count,Gs_VEL(:,ind_A)];
            temp2=[ode_sol.x'+time_count2,ode_sol.y(ind_gs,:)'];
            photo_rate=[photo_rate;temp];
            gs_conc=[gs_conc;temp2];
            time_count=time_count+ode_sol.x(end);
            time_count2=time_count2+ode_sol.x(end);
        end
    else
        simA(i)=1e5;
        simGs(i)=1e5;
