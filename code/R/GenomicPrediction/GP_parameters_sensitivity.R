...
predicted_traits <- read.csv(paste0(path,"results/sensitivity_results/predicted_parameters_LOO_",method,".csv"))
rownames(predicted_traits) <- predicted_traits$X
predicted_traits <- predicted_traits[,-1]
...
for (a in 29:68){
  ...
  end_time <- Sys.time()
  end_time - start_time
  write.csv(predicted_traits,paste0(path,"results/predicted_parameters_LOO_",method,".csv"),row.names = TRUE)
}
