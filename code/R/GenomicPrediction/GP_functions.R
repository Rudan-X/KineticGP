...
calculate_BLUP <- function(df_orig){
  ind <- which(colnames(df_orig)%in%c("Year","Accession"))
  var22 <- df_orig[df_orig$Year==2022,]
  var23 <- df_orig[df_orig$Year==2023,]
  
  common_lines<-intersect(var22$Accession,var23$Accession)
  
  var22 <- var22[match(common_lines,var22$Accession),]
  var23 <- var23[match(common_lines,var23$Accession),]
  ...
  for (i in 1:nvar){
    value<-as.numeric(c(var22[,i],var23[,i]))
  ...
  df <- cbind(data.frame(Accession=common_lines),as.data.frame(BLUPS))
  colnames(df) <- c("Accession",colnames(var22))
  return(df)
}
