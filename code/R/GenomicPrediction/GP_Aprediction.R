...
predicting_photosynthesis<-function(scale,method){
  ...
  write.csv(predicted_traits,paste0("results/GP_Aprediction/",scale,"_predicted_photosynthesis_",method,".csv"),row.names = FALSE)

}

predicting_photosynthesis_BLUPs<-function(scale,method){
  ...
  measBLUP <-  calculate_BLUP (measure)

  rownames(measBLUP)<-measBLUP$Accession
  training_lines<-measBLUP$Accession
  training_data<-measBLUP[,-1]
  ...
  testing_data <- read.csv("data/processed_data/Testing_AQcurves_years22&23_accession.csv")
  testingBLUP <- calculate_BLUP(testing_data)
  
  testing_lines <- testingBLUP$Accession
  ...
  write.csv(predicted_traits,paste0("results/GP_Aprediction/BLUP_",scale,"_predicted_photosynthesis_",method,".csv"),row.names = FALSE)
...
predicting_photosynthesis21<-function(scale,method){
  ...
  write.csv(predicted_traits_train,paste0("results/GP_Aprediction/",scale,"_trained_photosynthesis_",method,"21.csv"),row.names = FALSE)
  write.csv(predicted_traits,paste0("results/GP_Aprediction/",scale,"_predicted_photosynthesis_",method,"21.csv"),row.names = FALSE)
