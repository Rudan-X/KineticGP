...
predicting_params_BLUP<-function(topX,scale,method){
  ...
  for (o in 1:10){
    for (f in 1:3){
      ...
      predicted_traits <- predicted_traits[1:(min(vmaxind23)-1),]
...
for (x in c(20)){
  predicting_params_BLUP(topX = x,scale = "original",method="rrBLUP") #
}
