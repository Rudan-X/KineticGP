...
df<-rbind(df,data.frame(Measured=measAQ$PAR_1800,Predicted=predAQ$PAR_1800,
                        Accession=measAQ$Accession,Model="Baseline model", MLmethod=method))
...
df$Model <- factor(df$Model, levels = c("KineticGP-10","Baseline model"))


ggplot(df, aes(y=Predicted,x=Measured, color=Model,shape=Model)) + # , color=Season,shape=Season
  ...
  stat_cor(method = "pearson", cor.coef.name = "r",label.y.npc = "bottom", show.legend = F , size=5)+
...
ggsave('figures_pre/GP_BLUP.png',width=10,height = 6)
