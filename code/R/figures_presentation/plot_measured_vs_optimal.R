...
colnames(df) <- c("Condition","Accession","Ameas","Asim")
...
for (c in 1:3){
  ...
  summary_long <- summary_df %>%
    tidyr::pivot_longer(
      cols = c(Mean_simA, lower_simA, upper_simA, Mean_measA, lower_measA, upper_measA ),
      names_to = c(".value", "source"),
      names_pattern = "(.*)_(simA|measA)"
    )
  
  summary_long$source <- factor(summary_long$source, levels=c("measA", "simA"))
  ...
  g[[c]] <- ggplot(summary_long, aes(x = Condition, color = source, shape = source)) +
    ...
    scale_color_manual(values = c("measA" = "#6A3D9A", "simA" = "#33A02C"),
                       labels = c("Measurement", "Gneotype-specific simulation"),
                       name = "") +
    scale_shape_manual(values = c("measA" = 1, "simA" = 5),
                       labels = c("Measurement", "Gneotype-specific simulation"),
                       name = "") +
...
ggsave('figures_pre/measured_vs_optimalsim.png',width=10,height = 6)
