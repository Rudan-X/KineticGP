...
for (c in 1:3){
  ...
  g[[c]] <- ggplot(summary_long, aes(x = Condition, color = source, shape = source)) +
    ...
    scale_color_manual(values = c("defA"="#3B5BA5","measA" = "#6A3D9A"),
                       labels = c("Maize-specific simulation (default)","Measurement"),
                       name = "") +
...
ggsave('figures_pre/measured_vs_defaultsim.png',width=10,height = 6)
