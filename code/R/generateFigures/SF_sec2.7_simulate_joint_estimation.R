...
df <- df_meas
df$A <- df_sim$value
colnames(df) <- c("Condition","Accession","A","Ameas")
...
y_axis <- list(expression("Photosynthetic rate"~(mu*mol/m^2/s)),
               expression("Stomatal conductance"~(mu*mol/m^2/s)),
               expression("Photosynthetic rate"~(mu*mol/m^2/s)))

x_axis <- list(expression("Ambient " ~ CO[2] ~ "(" * mu * mol/mol * ")"),
               expression("Aambient " ~ CO[2] ~ "(" * mu * mol/mol * ")"),
