...
for (method in c("rrBLUP","BGLR")){ #,
  ...
  for (i in 1:length(tops1)){
    topX <- tops1[i]
    files <- c(files,paste0("results/KineticGP_Asimulation/simulatedA_control_mean_and_ACCU_top",topX,"_",scale,"_",method,".csv"))
...
ggplot(df, aes(y=Predicted,x=Measured, color=Season,shape=Season)) +
  ...
  scale_color_manual(name="Season",values = c("2022"="#1F77B4","2023"="#FF8C69")) +
...
ggsave(paste0("Figures/SFsec5.3_GP_control.png"),width =6, height = 10)
