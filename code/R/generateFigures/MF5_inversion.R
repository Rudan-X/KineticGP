...
all_simAsat <- matrix(nrow = 128, ncol = ncond)

conditions <- c()
for (r in 1:length(RHs)){
  filen <- paste0("results/KineticGP_Asat_env/simulatedA_top",topX,"_",scale,"_",method,"_RH",RHs[r],".csv")
...
measured_rates <- measAQ21[ind_complete,3]
...
dataA <- data.frame(Measured = meas_values, Simulated = sim_values, Condition = field_cond, Genotype = genotypes, type="Adjusted PAR")
...
genotypes <- paste0("",seq(1,nvar))
dataB <- data.frame(Measured = meas_values, Simulated = sim_values, Condition = field_cond, Genotype = genotypes, type="Adjusted")
...
genotypes <- paste0("",seq(1,nvar))
dataC <- data.frame(Measured = meas_values, Simulated = sim_values, Condition = field_cond, Genotype = genotypes, type="Adjusted")
...
save.image(file = "results/RData/season21_inversion.RData")


top_row = ggarrange(g1, g2, ncol = 2, labels = c("a", "b"))
bottom_row = ggarrange( g3, labels = c("c"))
ggarrange(top_row, bottom_row , ncol = 1, heights = c(1,0.8) )

ggsave(paste0("Figures/MF5_inversion.png"),width =9.5, height = 10)
