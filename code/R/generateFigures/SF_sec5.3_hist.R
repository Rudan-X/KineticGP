...
df2 <- melt(df)
df2$Season<-factor(df$Season,levels=unique(df$Season))

ggplot(df2, aes(x=value, color=Season)) +
  geom_histogram(fill="white", position="dodge")+
  theme(legend.position="top") +facet_wrap(~variable,ncol=1) +
  geom_vline(data=mu, aes(xintercept=grp.mean, color=Season),
             linetype="dashed")

mu <- ddply(df, "Season", summarise, grp.mean=mean(Measured))
mu <- ddply(df, "Season", summarise, grp.mean=mean(Predicted))
...
ggsave(paste0("Figures/SFsec5.2_GP_control",method,".png"),width =7, height = 7)
